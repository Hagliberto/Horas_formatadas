...
def criar_janela():
    ...
    layout = [
        ...
        [sg.Text('Nome do arquivo de saída para importação no Protheus:', size=(40, 1)), sg.Input(
            key='-NOME-', default_text='hr_oitchau')],
...
def processar_arquivos(arquivos, nome_arquivo_saida, pasta_destino):
    ...
    if not dados_completos.empty:
        ...
        if not dados_completos.empty:
            ...
            # Salva o arquivo de saída em formato Excel
            arquivo_saida_excel = os.path.join(
                pasta_destino, f'{nome_arquivo_saida}.xlsx')
            dados_completos.to_excel(
                arquivo_saida_excel, index=False, header=False)

            # Salva o arquivo de saída em formato CSV
            arquivo_saida_csv = os.path.join(
                pasta_destino, f'{nome_arquivo_saida}.csv')
            dados_completos.to_csv(
                arquivo_saida_csv, index=False, header=False, sep=';')
            ...
            for verba in dados_completos[1].unique():
                ...
                nome_arquivo = os.path.join(
                    pasta_destino, f'Verba_{verba}.xlsx')
            ...
            for verba in dados_completos[1].unique():
                dados_verba = dados_completos[dados_completos[1] == verba]
                nome_arquivo = os.path.join(
                    pasta_destino, f'Verba_{verba}.csv')
                dados_verba.sort_values([0, 1], inplace=True)
                dados_verba.to_csv(nome_arquivo, index=False, header=False, sep=';')
    ...
    if not dados_completos.empty:
        ...
        if not dados_completos.empty:
            ...
            arquivo_saida = os.path.join(
                pasta_destino, f'{nome_arquivo_saida}.csv')
            dados_completos.to_csv(
                arquivo_saida, index=False, header=False, sep=';')

            arquivo_saida = f'{nome_arquivo_saida}.xlsx'
            dados_completos.to_excel(
                arquivo_saida, index=False, header=False)
            
            arquivo_saida = f'{nome_arquivo_saida}.csv'
            dados_completos.to_csv(
                arquivo_saida, index=False, header=False, sep=';')
            ...
            for verba in dados_completos[0].unique():
                dados_verba = dados_completos[dados_completos[0] == verba]
                nome_arquivo = os.path.join(
                    pasta_destino, f'Horas_da_Matricula_{verba}.csv')
                dados_verba.sort_values([0, 1], inplace=True)
                dados_verba.to_csv(
                    nome_arquivo, index=False, header=False, sep=';')
